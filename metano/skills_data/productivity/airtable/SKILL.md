...
4. Store the token in `~/.claude/metano/.env` (或 metano 环境配置):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.claude/metano/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
