...
def introspect_and_report() -> dict:
    ...
    try:
        ...
        from .evo_models import add_proposal, get_proposals, normalize_proposal_content
        # Deduplicate proposals by normalized content (ignoring line numbers) so
        # the same finding is not re-queued on every scan once its line shifts.
        existing_proposals = {
            (p['proposal_type'], normalize_proposal_content(p['content']))
